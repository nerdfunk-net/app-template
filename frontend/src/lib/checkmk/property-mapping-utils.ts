...
export function initializePropertyMappings(
  ...
  if (deviceMetadata?.customFields) {
    deviceMetadata.customFields.forEach(cf => {
  ...
  Object.keys(attrs).forEach(key => {
    if (key.startsWith('tag_')) {
      ...
      if (attrPath) {
        ...
        deviceField = attrPath.split('.')[0] || 'no_mapping' // Get first part (e.g., "status")
        isCore = true
      } else {
      ...
      if (customFieldKeys.has(cleanKey) && deviceField === 'no_mapping') {
        ...
        deviceField = `custom_field_${cleanKey}`
      }
      // Otherwise use 'no_mapping' - attribute won't be synced

      // Mark as core if it maps to a core Nautobot attribute
      const isCoreNautobotField = ['name', 'primary_ip4', 'location', 'status', 'role', 'device_type', 'platform'].includes(deviceField)
...
export function resolveId(
  ...
  if (!deviceMetadata || !value) return String(value)

  const valueStr = String(value).toLowerCase()

  // Map field to metadata array
  const fieldMappings: Record<string, Array<{ id: string; name: string }>> = {
    'location': deviceMetadata.locations,
    'role': deviceMetadata.roles,
    'status': deviceMetadata.statuses,
    'device_type': deviceMetadata.deviceTypes,
    'platform': deviceMetadata.platforms,
  }
...
export function buildDevicePayload(
  ...
  Object.entries(propertyMappings).forEach(([_checkMkKey, mapping]) => {
    const { deviceField, value } = mapping

    // Skip fields with no mapping
    if (deviceField === 'no_mapping') {
      return
    }

    if (deviceField.startsWith('custom_field_')) {
      // Custom field
      const fieldKey = deviceField.replace('custom_field_', '')
      customFields[fieldKey] = String(value)
    } else {
      // Standard field - resolve ID if needed
      const resolvedValue = ['location', 'role', 'status', 'device_type', 'platform'].includes(deviceField)
        ? resolveId(deviceField, value, deviceMetadata)
        : value

      devicePayload[deviceField] = resolvedValue
    }
  })
